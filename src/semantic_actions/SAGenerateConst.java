...
public class SAGenerateConst implements SemanticAction {
    ...
    public Token execute(ReaderBuffer fc, StringBuilder lexeme, char lastCharacter) {
        ...
        if (constant >= -Math.pow(2, MAX_BITS_INT) && constant <= Math.pow(2, MAX_BITS_INT) - 1) { // it's an integer
            token = new Token(SymbolTable.getInstance().getID("integer const"), Long.toString(constant), INT);
        } else {            //  it's a long
            if (constant < -Math.pow(2, MAX_BITS_LONG)) {  // overflow
                System.out.println("    Lexical Warning at line " + fc.getLine() + ": constant out of range. Min defined " + Math.pow(2, -MAX_BITS_LONG) + ".");
                fc.addWarning();
                return null;
            } else {
                if (constant > Math.pow(2, MAX_BITS_LONG) - 1) {
                    System.out.println("    Lexical Warning at line " + fc.getLine() + ": constant out of range. Max defined " + (Math.pow(2, MAX_BITS_LONG) - 1) + ".");
                    fc.addWarning();
                    return null;
                }
            }
            token = new Token(SymbolTable.getInstance().getID("long const"), Long.toString(constant), LONG); // 
        }
