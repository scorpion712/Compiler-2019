...
//#line 497 "gramatica.y"
    ...
    public void verificar_rango(String cte, long rango) {
        if (Long.parseLong(cte) == rango) {
            ...
            SymbolTable.getInstance().modificarLexema(cte, Long.toString(rango - 1));
        }
    }

    public void verificar_negativo(String cte) {
        String nuevoLex = "-" + cte;
        Token viejo = SymbolTable.getInstance().getSymbol(cte);
        if (!SymbolTable.getInstance().containsLexeme(nuevoLex)) {
            Token t = new Token(viejo.getID(), nuevoLex, viejo.getDescription() + " negativa");
            SymbolTable.getInstance().addSymbol(t);
        }
        int contador = Integer.parseInt(viejo.getAttribute("contador")) - 1;
        if (contador == 0) {
            SymbolTable.getInstance().remove(viejo.getLexeme());
            System.out.println("constante borrada");
        } else {
            viejo.addAttribute("contador", String.valueOf(contador));
        }
    }
    ...
    public void setearTipo(String tipo, Nodo variable) {
        if (variable != null) {
            ...
            if (variable.esHoja()) {
                Token tk = SymbolTable.getInstance().getSymbol(variable.getNombre());
                tk.setType(tipo);
            }
...
//########## USER-SUPPLIED ACTIONS ##########
                ...
                break;
                case 43: //#line 151 "gramatica.y"
                {
                    yyval = val_peek(0);
                }
                break;
                case 44: //#line 157 "gramatica.y"
                {
                ...
                break;
                case 111: //#line 447 "gramatica.y"
                {
                    yyval = val_peek(0);
                    /*System.out.println("- de factor a termino con "+((Nodo)$1.obj).getNombre());*/
                }
                break;
                case 112: //#line 453 "gramatica.y"
                {
                    Token tk = SymbolTable.getInstance().getSymbol(val_peek(0).sval);
                    Nodo nodo = new Nodo(val_peek(0).sval, tk.getType());
 ...
 /*System.out.println("- nuevo nodo "+nodo.getNombre()+" tipo "+nodo.getTipo());*/
                ...
                break;
                case 116: //#line 480 "gramatica.y"
                {
                    String cte = val_peek(0).sval;
                    verificar_negativo(cte);
                    yyval = new ParserVal(new Nodo('-' + val_peek(1).sval, "integer"));
                    Token tk = SymbolTable.getInstance().getSymbol(val_peek(1).sval);
                    tk.setType("integer");
                }
                break;
                case 117: //#line 487 "gramatica.y"
                {
                    String cte = val_peek(0).sval;
                    verificar_negativo(cte);
...
//## Constructors ###############################################
     ...
     *
     */
    public Parser(LexerAnalyzer al) {
        this.al = al;
    }
