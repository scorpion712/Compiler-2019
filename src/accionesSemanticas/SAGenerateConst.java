...
public class SAGenerateConst implements SemanticAction {
    ...
    // this two constants may have the same string acording to INT and LONG values in identifiers map in SymbolTable
    ...
    public Token execute(ReaderBuffer fc, StringBuilder lexeme, char lastCharacter) {
        ...
        if (constant >= Math.pow(2, -MAX_BITS_INT) && constant <= Math.pow(2, MAX_BITS_INT) - 1) { // it's an integer
            ...
            if (constant < Math.pow(2, -MAX_BITS_ULONG)) {  // overflow
                System.err.println("Lexical Warning at line " + fc.getLine() + ": constant out of range. Min defined " + Math.pow(2, -MAX_BITS_ULONG) + ".");
                constant = (long) Math.pow(2, -MAX_BITS_ULONG);
            } else {
                if (constant > Math.pow(2, MAX_BITS_ULONG) - 1) {
                    System.err.println("Lexical Warning at line " + fc.getLine() + ": constant out of range. Max defined " + (Math.pow(2, MAX_BITS_ULONG) - 1) + ".");
